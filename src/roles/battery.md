...
7. Vacuity-by-mutation (field-proven, twice): pick the target's most load-bearing assertions and
   MUTATE the guarded behavior in a SCRATCH COPY (never the target itself) so it is definitely
   broken — then run the target's own suite against the mutant. A suite that stays green under
   the mutation is not testing that behavior; the finding carries the mutation diff + the green
   run as its reproduction. Priority mutants: disable a dedupe/cache guard, make a budget counter
   never increment, swap a keying dimension (per-session → shared). Red-first history does NOT
   immunize a suite against this — red-before-build proves the suite CAN fail, not that it fails
...
7. Transcript-prose laundering (field-proven): when the target's evidence includes an LLM-DRIVEN
   run (an agent transcript, a "live test" narrative), only the session log's TOOL-RESULT blocks
   are admissible ground truth — driver models fabricate plausible status quotes in prose,
   including verbatim-looking strings in the target's own output format that no tool ever
   emitted. Re-extract the claimed evidence from the raw log's tool results; a claim sourced
   only to model prose is a flag, and evidence that dissolves on re-extraction is a finding.
