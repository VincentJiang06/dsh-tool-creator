...
- [ ] **Mutation-proof the load-bearing assertions.** Red-before-build proves the suite CAN fail;
      it does not prove the suite fails when the FEATURE later breaks — a test can be born red,
      turn green, and still be vacuous (field incidents, twice: a dedupe test that stayed green
      when dedupe consumed the budget it guarded; a disclosure test asserting a disjunction that
      was always true). For each behavior the dossier leans on, break that behavior once in a
      scratch copy and record that the suite goes red against the mutant. A mutation the suite
      survives means you fix the TEST before returning, and the mutation+kill goes in the layer
      notes as evidence the test discriminates.
...
- Final self-audit (each "no" goes into the dossier as an honest note, not a silent gap):
  ...
  - Does every NUMBER stated in the build's prose (README test counts, badge-style claims,
    version strings, case totals) come from re-running the live command at dossier time, never
    from memory? Metadata drift recurred across consecutive releases in the field — the same
    project shipped a stale test-count badge twice in a row. Diff prose against reality
    mechanically; it is the cheapest check in this list and the most-repeated failure.
