...
test('runtime cache (__pycache__) generated by a command is ignored — no false unlisted-files / tree-unchanged (P3-b)', () => {
  const { dir, manifest } = buildGoodArtifact();
  try {
    // A real python harness regenerates scripts/__pycache__/*.pyc on every run. reverify must ignore
    // it (byte-identical to assemble's walk exclusion) or a legitimate re-verify false-REDs.
    manifest.reverify.commands.push({
      id: 'make-pyc', cwd: '.', expectedExit: 0, description: 'writes a runtime cache file',
      argv: ['python3', '-c', "import os;os.makedirs('scripts/__pycache__',exist_ok=True);open('scripts/__pycache__/toy.cpython-312.pyc','w').close()"],
    });
    writeManifest(dir, manifest);
    const { status, json } = runReverify(dir);
    assert.equal(status, 0, '__pycache__ generated by a command must not fail tree-unchanged/unlisted-files');
    assert.equal(statusOf(json, 'cmd:make-pyc').status, 'PASS');
    assert.equal(statusOf(json, 'tree-unchanged').status, 'PASS');
    assert.equal(statusOf(json, 'unlisted-files').status, 'PASS');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
